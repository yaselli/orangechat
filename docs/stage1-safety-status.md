...
- 模型、语音、搜索、MCP（含 OAuth）、QuickJS 插件 fetch、原生文件/网页/壁纸/Rootfs 下载接入共同许可检查。HTTPS 保持可用；仅 localhost、127.0.0.1、::1 默认允许 HTTP。局域网和公网 HTTP/WS 均需按地址及端口确认，子域和其他端口不自动继承。
- 设置页新增“HTTP 连接”。旧 HTTP 配置不会被改写或删除，但升级后的首次请求会被拦截，用户需在设置确认该地址后重试；此兼容性变化必须在发包说明中告知。许可仅保存在本机 SharedPreferences，当前系统备份白名单不包含它；导入设置不会导入许可。
- 应用拦截器在 DNS/正文发送前检查初始请求，网络拦截器检查重定向目标；HTTPS/HTTP 跨协议自动重定向关闭。WebSocket 禁止自动重定向，因为 OkHttp 的 WebSocket 路径不运行网络拦截器。已有流式连接在撤销后不强制断开，后续请求重新检查。
- 仅保留最近 20 个被拦截 origin，不记录路径、查询、凭据或正文；错误信息不回显原 URL。异常退出前各下载响应使用 use 关闭。
- 边界：Manifest 的 usesCleartextTraffic 仍为 true，以支持用户运行时授权的任意 HTTP 域名。静态 XML 无法表达这份运行时清单；不能称为全应用/系统级禁止明文。WebView 网页脚本、终端/子进程、第三方原生媒体库不受本许可控制，设置页明确展示这一点。这些网络出口仍需专门处理，不能宣称 HTTP 风险全部结清。
- 新增策略与传输测试覆盖 HTTPS/回环例外、端口/子域隔离、撤销、凭据拒绝、初始请求在 DNS 前阻断、跨 origin 重定向不发送请求头。真机仍需验证旧 HTTP 接口授权后重试和撤销行为。
