...
## HTTP 地址审批撤回（2026-09-10）

按用户决定取消 HTTP/WS 地址许可、请求拦截和设置页审批入口。已有和新配置的 HTTP 接口均无需额外授权；旧本地许可数据不再读取，不影响连接。Manifest 保持允许明文连接。
保留 HTTPS 的证书验证、禁止 HTTPS/HTTP 跨协议自动重定向，以及独立的日志脱敏、密钥加密保护。下载响应仍由调用方关闭。
传输回归测试改为验证无许可、无初始化时 HTTP 请求及跨地址 HTTP 重定向均可成功。构建和测试结果以本次工作流为准。
